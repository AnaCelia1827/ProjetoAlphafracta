...
describe("useFeeHistory", () => {
  ...
  it.each([5, 15, 60, 360, 1440] as const)(
    ...
    async (rangeMinutes: HistoryRangeMinutes) => {
      ...
      expect(baselineTo.getTime() - baselineFrom.getTime()).toBe(
        1440 * 60_000,
      );

      if (rangeMinutes !== 1440) {
        const [selectedFrom, selectedTo] =
          fetchAllFeeHistoryMock.mock.calls[1]!;
        expect(selectedTo).toBe(baselineTo);
        expect(selectedTo.getTime() - selectedFrom.getTime()).toBe(
          rangeMinutes * 60_000,
        );
      }
  ...
  it("does not label a previous range as the newly selected range", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ rangeMinutes }) => useFeeHistory(rangeMinutes),
