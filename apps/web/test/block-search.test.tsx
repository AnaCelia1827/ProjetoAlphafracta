...
describe('block search', () => {
  it('submits a block number only on Enter', async () => {
    ...
    function Harness() {
      ...
      return <DashboardFilters search={search} onSearchChange={setSearch} onSearch={onSearch} />;
    }
  ...
  it('loads a canonical block through the versioned endpoint', async () => {
    const fetchMock = vi.fn().mockResolvedValue(Response.json({ data: blockFixture }));
    vi.stubGlobal('fetch', fetchMock);
