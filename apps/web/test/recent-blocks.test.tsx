...
describe('recent block actions', () => {
  it('renders at most ten blocks with the official icon and history link', () => {
    ...
    render(<RecentBlocks blocks={blocks} searchedBlock={null} onBackToLive={vi.fn()} />);
  ...
  it('opens the backend-provided Etherscan URL', async () => {
    ...
    render(
      <RecentBlocks blocks={[blockViewFixture]} searchedBlock={null} onBackToLive={vi.fn()} />,
    );
  ...
  it('shows a search error without hiding the live blocks', () => {
    ...
    expect(screen.getByText('Informe um número de bloco ou hash Ethereum válido.')).toBeVisible();
    expect(
